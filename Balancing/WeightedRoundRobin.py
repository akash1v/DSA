...
class RoundRobin:
    def __init__(self, no_of_machines, weights):
        self.machines = [Machine(i, weights[i]) for i in range(no_of_machines)]
        self.timer = 0
        self.w_id = 0
        self.m_id = 0
        self.max_id = len(weights)
    ...
    def execution(self, tasks):
        while tasks:
            ...
            self.w_id += 1
            if(self.w_id >= self.machines[self.m_id].weight):
                self.w_id = 0
                self.m_id += 1
                if(self.m_id >= self.max_id -1):
